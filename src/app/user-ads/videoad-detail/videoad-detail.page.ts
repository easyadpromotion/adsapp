...
@Component({
  selector: 'app-videoad-detail',
  templateUrl: './videoad-detail.page.html',
  styleUrls: ['./videoad-detail.page.scss'],
})
export class VideoadDetailPage implements OnInit {
  data={};
  showComment=false;
  alreadyEarned=false;
  liked=false;
  ...
  addMoney(){
    this.firebase.readData('amountConfig').subscribe(res=>{
      ...
      console.error(amountConfig,this.data,amountCal,json['amount'])
      json['status']=1;
